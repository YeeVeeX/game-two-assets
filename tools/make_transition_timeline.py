#!/usr/bin/env python3
"""Tick-accurate transition-smoothing A/B timeline sheet (sprint 6, calibration-v6).

Extends the banked v5 timeline protocol to the two-way transition comparison
under identical pinned timing (manifests/render-reference.json
`attack_timing`, captured read-only from the pinned game commit). Timelines
share the banked-winner five-state grammar everywhere except the two
transition ticks, so those ticks are the only variable:

- timeline A (incumbent): the v5-banked winner grammar unmodified - a0 held
  all 5 windup ticks at the pinned -3px, k0 held 4 active ticks at +6px, r0
  held all 8 recovery ticks at 0. Both tested boundaries are single-tick
  full pose swaps.
- timeline B (candidate): w0 occupying windup tick 1 (t15, at -3) then a0
  held t16-t19; s0 occupying recovery tick 1 (t24, at 0) then r0 held
  t25-t31. In-betweens consume pinned ticks, never add them.

The a0->k0 release boundary (t19->t20, +9px) is deliberately untouched in
BOTH timelines - the banked v4 salience. The -6px k0->recovery position
return likewise remains in both; this sheet smooths only the pose component.
...
- APPROACH: idle_pre + the 13-tick walk step (identical in A/B, one row per
            zone)
- ATTACK:   Z1 A/B then Z2 A/B stacked (t14-t33), both zone palettes
- 2X:       the six transition-region ticks of timeline B - t14 (arrival),
            t15 (w0), t16 (first a0), t23 (last k0), t24 (s0), t25 (first
            r0): both boundaries under test at 2x
- 4X:       one strip per boundary, the X | M | Y triplet at its real
            timeline offsets - ONSET idle@0 | w0@-3 | a0@-3 and SETTLE
            k0@+6 | s0@0 | r0@0 - at 4x nearest-neighbor (pixel diagnosis)
- FILM:     static strip idle | f0-f3 | a0 | k0 | r0 | w0 | s0 over both
            zone palettes (ten columns)
- DIFF:     each in-between vs its two endpoints at 2x (derived diagnostic
            via the banked diff_pixels - not a creature cell)
- GRAMMAR:  nine static cells via the banked tell_cell - idle | f1 | w0@-3 |
            a0 | a0@-3 | k0 | k0@+6 | s0@0 | r0@0 (the in-betweens inserted
            at their timeline positions)

Every creature cell is recorded in a machine-readable manifest consumed by
tools/transition_metrics.py for composition-purity and tick-math
verification. Layout is fixed; regeneration is byte-identical. Banked tools
are imported unmodified. Optional APNG viewing aids (A | B side by side, 4x
...
FACINGS = ("down", "right")
TIMELINES = ("A", "B")
WINDUP_INBETWEEN = "w0"   # timeline B, windup tick 1 only
SETTLE_INBETWEEN = "s0"   # timeline B, recovery tick 1 only
WINDUP_POSE = "a0"        # the banked v4 winner, held in both timelines
RECOVERY_POSE = "r0"      # the banked v5 winner, held in both timelines
STRIP = ("idle", "f0", "f1", "f2", "f3", "a0", "k0", "r0", "w0", "s0")
DIFF_PAIRS = (
    ("w0", "idle"), ("w0", "a0"), ("s0", "k0"), ("s0", "r0"),
)
BOUNDARIES = ("onset", "settle")
TWOX_SCALE = 2
FOURX_SCALE = 4
APNG_SCALE = 4
...
def build_plan(reference: dict) -> dict:
    ...
    Poses are identical across A/B outside the two transition ticks (windup
    tick 1 and recovery tick 1) - the isolation that makes the comparison
    decidable.
    """
    ...
    for index in range(windup):
        poses = (
            {"A": WINDUP_POSE, "B": WINDUP_INBETWEEN}
            if index == 0
            else shared(WINDUP_POSE)
        )
        add("windup", poses, TILE + lunge["windup_px"], lunge["windup_px"])
    for _ in range(active):
        add("active", shared("k0"), TILE + lunge["active_px"], lunge["active_px"])
    for index in range(recovery):
        poses = (
            {"A": RECOVERY_POSE, "B": SETTLE_INBETWEEN}
            if index == 0
            else shared(RECOVERY_POSE)
        )
...
def transition_ticks(plan: dict) -> list[dict]:
    """The six transition-region ticks: arrival, w0, first a0 (the onset
    boundary) and last k0, s0, first r0 (the settle boundary)."""
    ticks = plan["ticks"]
    ...
    arrival = next(t for t in ticks if t["tick"] == plan["arrival_tick"])
    return [
        arrival, windup[0], windup[1],
        active[-1], recovery[0], recovery[1],
    ]


def boundary_triplets(plan: dict) -> dict[str, list[dict]]:
    """Per boundary: the X | M | Y triplet at real timeline positions."""
    six = transition_ticks(plan)
    return {"onset": six[:3], "settle": six[3:]}
...
class TransitionTimelineSheet:
    ...
    def __init__(
        ...
        self.poses = {
            facing: load_poses(
                transition_dir, recovery_dir, anticipation_dir, attack_dir,
                walk_dir, idle_dir, facing,
            )
    ...
    def facing_height(self, facing: str) -> int:
        ...
        total += 2 * (ruler + cell_size(facing, 2, FOURX_SCALE)[1] + GUTTER)  # 4X
    ...
    def twox_row(self, cv, y: int, facing: str) -> int:
        transitions = transition_ticks(self.plan)
    ...
    def fourx_rows(self, cv, y: int, facing: str) -> int:
        triplets = boundary_triplets(self.plan)
        w, h = cell_size(facing, 2, FOURX_SCALE)
        step_w = w + GUTTER
        for boundary in BOUNDARIES:
            ticks = triplets[boundary]
            draw_text(cv, MARGIN_LEFT, y, f"{boundary.upper()} 4X")
            for index, tick in enumerate(ticks):
                draw_text(
    ...
    def diff_row(self, cv, y: int, facing: str) -> int:
        """Derived diagnostic (banked diff_pixels): each in-between vs its two
        endpoints. Not a creature cell - excluded from the purity manifest by
        design."""
        draw_text(cv, 2, y + TILE - 2, "DIFF")
        for index, (inbetween, endpoint) in enumerate(DIFF_PAIRS):
            x = MARGIN_LEFT + index * (TILE * TWOX_SCALE + GUTTER)
            cv.fill_rect(x, y, TILE * TWOX_SCALE, TILE * TWOX_SCALE, (30, 30, 30, 255))
            cv.blit_scaled(
                motion.diff_pixels(
                    self.poses[facing][endpoint], self.poses[facing][inbetween]
                ),
                x, y, TWOX_SCALE,
            )
            draw_text(
                cv, x, y + TILE * TWOX_SCALE - 6,
                f"{inbetween.upper()} {endpoint.upper()}",
            )
        return y + TILE * TWOX_SCALE + GUTTER

    def grammar_row(self, cv, y: int, facing: str) -> int:
        """The banked static grammar control row with the in-betweens inserted
        at their timeline positions: w0 before the coil pair (windup tick 1),
        s0 before the settle (recovery tick 1)."""
...
def build_apng_frames(sheet: TransitionTimelineSheet, facing: str) -> list[Rgba8Canvas]:
...
def main(argv: list[str] | None = None) -> int:
    ...
    sheet = TransitionTimelineSheet(
        args.transition_exports, args.recovery_exports, args.anticipation_exports,
