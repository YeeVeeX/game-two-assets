#!/usr/bin/env python3
"""Deterministic completion-rise metrics and validator for the sprint-7 sheet.

Measures and machine-checks the pre-registered calibration-v7 bars
(reviews/calibration-v7/rationale.md):

1. bridging bar on export bytes, per facing - max(d(r0,x0), d(x0,idle)) <
   d(r0,idle): the ready-again boundary's largest single-tick silhouette
   jump strictly decreases (full decomposition reported);
2. nearest-neighbor bar - among the four grammar states {idle, a0, k0, r0},
   x0's two smallest deltas are exactly {r0, idle} (deltas to every walk
   frame AND to the banked w0/s0 bridges reported - no floor on bridges);
3. identity ceiling - every x0 delta < the 44.44% cross-facing reference;
   feet-contact row within +-1px of the idle baseline;
4. frozen-state protection - every consumed export hashes to its banked
   release.json SHA-256 (calibration-v0..v3, v5, v6, v7: 22 files), so the
   six banked frame-sets are byte-untouched; the a0->k0 release boundary
   AND the banked w0/s0 transition ticks (t15, t24) are machine-compared
   IDENTICAL in timelines A and B (subsumed by tick-identity - checked
...
7. tick math exact - windup = 5 cells at -3 in BOTH timelines (w0 x1 +
   a0 x4, the banked v6 winner), active = 4 at +6 (k0, both), recovery = 8
   at 0 (A: s0 x1 + r0 x7; B: s0 x1 + r0 x6 + x0 x1), walk = 13 at
   independently recomputed smoothstep positions; timelines tick-identical
   except t31 (the only differing tick).

Also reports the extended boundary-jump table across A and B - rows
(14,15), (15,16), (19,20), (23,24), (24,25), (30,31), (31,32), (32,33) with
pose delta + position delta and sharpest-single-tick attributions - and the
derived hold durations including the cumulative r0-hold disclosure
(133.3 -> 116.7 -> 100.0 ms across v5/v6/v7). `--check` exits nonzero on
any violation.
"""
...
import make_rise_timeline as timeline  # noqa: E402
...
INBETWEENS = {"x0": ("r0", "idle")}
BANKED_BRIDGES = ("w0", "s0")
BOUNDARY_ROWS = (
    (14, 15), (15, 16), (19, 20), (23, 24), (24, 25), (30, 31), (31, 32),
    (32, 33),
)
...
POSE_FILES = {
    ...
    "x0": ("rise_dir", "player_1_lane_b_attack_{facing}_x0.png"),
}
RELEASE_IDS = (
...
class RiseMetricsError(ValueError):
...
def inbetween_metrics(
    ...
    r0_stats = frame_stats(frames["r0"])
    stats = frame_stats(m)
    ...
    bridge_deltas = {
        bridge: pair_stats(frames[bridge], m)["popping_pct"]
        for bridge in BANKED_BRIDGES
    }
    ...
    return {
        ...
        "mass_chain_r0_x0_idle": [
            r0_stats["mass"], stats["mass"], idle_stats["mass"],
        ],
        "mass_monotone": r0_stats["mass"] < stats["mass"] < idle_stats["mass"],
        "bridging": {
            "d_x_m_pct": d_xm,
            "d_m_y_pct": d_my,
            "d_x_y_pct": d_xy,
        ...
        "max_delta_pct": max(
            [*grammar_deltas.values(), *walk_deltas.values(),
             *bridge_deltas.values()]
        ),
...
# -- boundary-jump table, release + banked-bridge preservation -----------------
    ...
    for facing, frames in frames_by_facing.items():
        ...
        for tl in timeline.TIMELINES:
            ...
            per_timeline[tl] = {
                "boundary_rows": [
                    j for j in jumps
                    if (j["from_tick"], j["to_tick"]) in BOUNDARY_ROWS
                ],
...
def banked_bridge_preservation(plan: dict) -> dict:
    """The banked v6 transition ticks must carry w0/s0 in BOTH timelines -
    subsumed by tick-identity outside t31, checked explicitly (defense in
    depth)."""
    ticks = plan["ticks"]
    windup_first = [t for t in ticks if t["phase"] == "windup"][0]
    recovery_first = [t for t in ticks if t["phase"] == "recovery"][0]
    return {
        "windup_tick_1": {
            "tick": windup_first["tick"],
            "poses": dict(windup_first["poses"]),
            "identical_and_banked": set(windup_first["poses"].values()) == {"w0"},
        },
        "recovery_tick_1": {
            "tick": recovery_first["tick"],
            "poses": dict(recovery_first["poses"]),
            "identical_and_banked": set(recovery_first["poses"].values()) == {"s0"},
        },
    }


def check_banked_bridges(preservation: dict) -> list[str]:
    failures = []
    for key, expected in (("windup_tick_1", "w0"), ("recovery_tick_1", "s0")):
        entry = preservation[key]
        if not entry["identical_and_banked"]:
            failures.append(
                f"{key} (t{entry['tick']}): poses {entry['poses']} are not the "
                f"banked {expected} in both timelines"
            )
    return failures
...
def durations_ms(constants: dict) -> dict:
    ...
    return {
        ...
        "ticks are the contract numbers; timeline B consumes the last pinned "
        "recovery tick for the rise in-between (never adds ticks)",
        "windup_ms": round(constants["windup_frames"] * TICK_MS, 1),
        "active_ms": round(constants["active_frames"] * TICK_MS, 1),
        "recovery_ms": round(recovery * TICK_MS, 1),
        "step_ms": round(constants["step_frames"] * TICK_MS, 1),
        "timeline_b_holds": {
            "s0_ticks": 1,
            "r0_hold_ticks": recovery - 2,
            "r0_hold_ms": round((recovery - 2) * TICK_MS, 1),
            "x0_ticks": 1,
        },
        "r0_hold_series_ms": {
            "v5_incumbent_8_ticks": round(8 * TICK_MS, 1),
            "v6_winner_7_ticks": round(7 * TICK_MS, 1),
            "v7_candidate_6_ticks": round(6 * TICK_MS, 1),
            "kb_follow_through_reference_ms": 150,
        },
    }
...
def reconstruct_cell(sheet: timeline.RiseTimelineSheet, cell: dict) -> Rgba8Canvas:
...
def check_purity(
    canvas: Rgba8Canvas, sheet: timeline.RiseTimelineSheet,
    dirs: dict[str, Path],
) -> dict:
    failures: list[str] = []
...
def check_tick_math(
    sheet: timeline.RiseTimelineSheet, reference: dict
) -> list[str]:
    failures: list[str] = []
    ...
    pose_sequences = {
        "windup": {
            tl: ["w0"] + ["a0"] * (windup_n - 1) for tl in timeline.TIMELINES
        },
        "active": {
        ...
        "recovery": {
            "A": ["s0"] + ["r0"] * (recovery_n - 1),
            "B": ["s0"] + ["r0"] * (recovery_n - 2) + ["x0"],
        },
    }
    offsets = {
    ...
    for facing in timeline.FACINGS:
        for zone in ("zone_1", "zone_2"):
            ...
            if transition != {31}:
                failures.append(
                    f"{facing}/{zone}: differing ticks {sorted(transition)} != "
                    f"[31] (t31 must be the only differing tick)"
                )
            for tick, cell_a in per_timeline["A"].items():
                ...
                if tick in transition and cell_a["pose"] == other["pose"]:
                    failures.append(
                        f"{facing}/{zone}/tick{tick}: rise tick shows the "
    ...
    expected_four = [t["tick"] for t in timeline.rise_ticks(sheet.plan)]
    twox = [c for c in sheet.cells if c["section"] == "twox"]
    for facing in timeline.FACINGS:
        ticks = [c["tick"] for c in twox if c["facing"] == facing]
        if ticks != expected_four:
            failures.append(
                f"{facing}: 2X boundary ticks {ticks} != {expected_four}"
            )
    expected_three = [t["tick"] for t in timeline.rise_triplet(sheet.plan)]
    fourx = [c for c in sheet.cells if c["section"] == "fourx"]
    for facing in timeline.FACINGS:
        ticks = [c["tick"] for c in fourx if c["facing"] == facing]
        if ticks != expected_three:
            failures.append(
                f"{facing}: RISE 4X ticks {ticks} != {expected_three}"
            )
        scales = {c["scale"] for c in fourx if c["facing"] == facing}
        if scales != {timeline.FOURX_SCALE}:
            failures.append(f"{facing}: RISE 4X scale {scales} != 4")
    return failures
...
def build_report(
    ...
    sheet = timeline.RiseTimelineSheet(
        rise_dir, transition_dir, recovery_dir, anticipation_dir, attack_dir,
        walk_dir, idle_dir, reference,
    )
    canvas = sheet.build()
    encoded = canvas.encode()
    second = timeline.RiseTimelineSheet(
        rise_dir, transition_dir, recovery_dir, anticipation_dir, attack_dir,
        walk_dir, idle_dir, reference,
    ).build().encode()
    ...
    return {
        "generated_by": "tools/rise_metrics.py",
        ...
        "banked_bridge_preservation": banked_bridge_preservation(sheet.plan),
...
def check_report(report: dict) -> list[str]:
    ...
    failures.extend(check_banked_bridges(report["banked_bridge_preservation"]))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--out", type=Path,
        default=ROOT / "reviews" / "calibration-v7" / "timeline-metrics.json",
    )
    ...
    except (RiseMetricsError, ValueError, OSError) as exc:
    ...
    if args.check:
        ...
        print(
            ...
            "preservation, banked-bridge preservation, byte-determinism, "
            "composition purity, tick math, banked export pins"
        )
    return 0
