from __future__ import annotations

import json
import sys
import tempfile
import unittest
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "tools"))

import make_recovery_timeline as timeline
import recovery_metrics
from pixel_spec import load_spec
from png_writer import Rgba8Canvas

REPO = Path(__file__).resolve().parents[1]
BODY = (235, 120, 40, 255)
ACCENT = (20, 14, 12, 255)


def write_rect(path: Path, x: int, y: int, width: int = 4, height: int = 4) -> None:
    """A minimal 32x32 export: body rect at (x, y) with one accent pixel."""
    canvas = Rgba8Canvas(32, 32)
    canvas.fill_rect(x, y, width, height, BODY)
    canvas.put(x + 1, y + 1, ACCENT)
    path.parent.mkdir(parents=True, exist_ok=True)
    canvas.save(path)


def load_reference() -> dict:
    return json.loads(
        (REPO / "manifests" / "render-reference.json").read_text(encoding="utf-8")
    )


class SyntheticRecoveryFixture(unittest.TestCase):
    """Distinct rects per pose so every cell is attributable byte-for-byte."""

    POSE_X = {
        "idle": 10, "f0": 11, "f1": 12, "f2": 13, "f3": 10,
        "a0": 14, "k0": 16, "r0": 18,
    }

    def setUp(self) -> None:
        self.temp = tempfile.TemporaryDirectory()
        self.addCleanup(self.temp.cleanup)
        self.root = Path(self.temp.name)
        self.recovery = self.root / "recovery"
        self.anticipation = self.root / "anticipation"
        self.attack = self.root / "attack"
        self.walks = self.root / "walks"
        self.idles = self.root / "idles"
        for facing in timeline.FACINGS:
            write_rect(
                self.idles / f"player_1_lane_b_idle_{facing}.png",
                self.POSE_X["idle"], 23,
            )
            for index in range(4):
                write_rect(
                    self.walks / f"player_1_lane_b_walk_{facing}_f{index}.png",
                    self.POSE_X[f"f{index}"], 23,
                )
            write_rect(
                self.anticipation / f"player_1_lane_b_attack_{facing}_a0.png",
                self.POSE_X["a0"], 25, 6, 3,
            )
            write_rect(
                self.attack / f"player_1_lane_b_attack_{facing}_k0.png",
                self.POSE_X["k0"], 21, 8, 6,
            )
            write_rect(
                self.recovery / f"player_1_lane_b_attack_{facing}_r0.png",
                self.POSE_X["r0"], 24, 7, 4,
            )
        self.reference = load_reference()

    def dirs(self) -> dict[str, Path]:
        return {
            "recovery_dir": self.recovery,
            "anticipation_dir": self.anticipation,
            "attack_dir": self.attack,
            "walk_dir": self.walks,
            "idle_dir": self.idles,
        }

    def sheet(self) -> timeline.RecoveryTimelineSheet:
        return timeline.RecoveryTimelineSheet(
            self.recovery, self.anticipation, self.attack, self.walks, self.idles,
            self.reference,
        )


class PlanTest(unittest.TestCase):
    def setUp(self) -> None:
        self.reference = load_reference()
        self.plan = timeline.build_plan(self.reference)

    def test_phase_counts_equal_pinned_constants(self) -> None:
        counts: dict[str, int] = {}
        for tick in self.plan["ticks"]:
            counts[tick["phase"]] = counts.get(tick["phase"], 0) + 1
        self.assertEqual(2, counts["idle_pre"])
        self.assertEqual(13, counts["walk"])
        self.assertEqual(5, counts["windup"])
        self.assertEqual(4, counts["active"])
        self.assertEqual(8, counts["recovery"])
        self.assertEqual(2, counts["idle_post"])
        self.assertEqual(34, len(self.plan["ticks"]))

    def test_recovery_span_is_eight_ticks_at_offset_zero_in_all_timelines(self) -> None:
        recovery = [t for t in self.plan["ticks"] if t["phase"] == "recovery"]
        self.assertEqual(8, len(recovery))
        for tick in recovery:
            self.assertEqual(0, tick["offset_px"])
            self.assertEqual(32, tick["axis_px"])
            self.assertEqual(
                {"A": "idle", "R": "r0", "C": "a0"}, tick["poses"]
            )

    def test_timelines_identical_except_the_recovery_pose(self) -> None:
        for tick in self.plan["ticks"]:
            poses = set(tick["poses"].values())
            if tick["phase"] == "recovery":
                self.assertEqual({"idle", "r0", "a0"}, poses)
            else:
                self.assertEqual(1, len(poses))

    def test_windup_holds_the_banked_coil_in_all_timelines(self) -> None:
        windup = [t for t in self.plan["ticks"] if t["phase"] == "windup"]
        self.assertEqual(5, len(windup))
        for tick in windup:
            self.assertEqual({"a0"}, set(tick["poses"].values()))
            self.assertEqual(-3, tick["offset_px"])
            self.assertEqual(29, tick["axis_px"])

    def test_active_holds_k0_at_plus_six(self) -> None:
        active = [t for t in self.plan["ticks"] if t["phase"] == "active"]
        self.assertEqual(4, len(active))
        for tick in active:
            self.assertEqual({"k0"}, set(tick["poses"].values()))
            self.assertEqual(6, tick["offset_px"])
            self.assertEqual(38, tick["axis_px"])

    def test_boundary_ticks_are_the_two_boundaries_under_test(self) -> None:
        ticks = [t["tick"] for t in timeline.boundary_ticks(self.plan)]
        self.assertEqual([23, 24, 31, 32], ticks)

    def test_walk_positions_follow_smoothstep_monotonically(self) -> None:
        walk = [t for t in self.plan["ticks"] if t["phase"] == "walk"]
        positions = [t["axis_px"] for t in walk]
        self.assertEqual(32, positions[-1])
        self.assertEqual(sorted(positions), positions)
        deltas = [b - a for a, b in zip([0] + positions, positions)]
        self.assertEqual(4, max(deltas))

    def test_attack_ticks_overlap_approach_at_arrival(self) -> None:
        self.assertEqual(14, self.plan["arrival_tick"])
        self.assertEqual(14, self.plan["approach_ticks"][-1]["tick"])
        self.assertEqual(14, self.plan["attack_ticks"][0]["tick"])
        self.assertEqual(20, len(self.plan["attack_ticks"]))


class SheetTest(SyntheticRecoveryFixture):
    def test_sheet_is_deterministic(self) -> None:
        self.assertEqual(self.sheet().build().encode(), self.sheet().build().encode())

    def test_sheet_geometry_is_fixed(self) -> None:
        canvas = self.sheet().build()
        self.assertEqual(1460, canvas.width)
        self.assertEqual(1740, canvas.height)

    def test_cell_manifest_covers_every_creature_cell(self) -> None:
        sheet = self.sheet()
        sheet.build()
        by_section: dict[str, int] = {}
        for cell in sheet.cells:
            by_section[cell["section"]] = by_section.get(cell["section"], 0) + 1
        self.assertEqual(
            {"approach": 60, "attack": 240, "twox": 8, "film": 32, "grammar": 14},
            by_section,
        )

    def test_recovery_cells_blit_the_per_timeline_pose_at_offset_zero(self) -> None:
        sheet = self.sheet()
        canvas = sheet.build()
        for tl, pose in timeline.RECOVERY_POSE.items():
            cells = [
                c for c in sheet.cells
                if c["section"] == "attack" and c["timeline"] == tl
                and c["phase"] == "recovery" and c["facing"] == "down"
                and c["zone"] == "zone_1"
            ]
            self.assertEqual(8, len(cells))
            cell = cells[0]
            self.assertEqual(0, cell["win_px"])
            self.assertEqual(pose, cell["pose"])
            x0, y0 = cell["rect"][0], cell["rect"][1]
            rect_y = {"idle": 23, "r0": 24, "a0": 25}[pose]
            self.assertEqual(
                BODY, canvas.get(x0 + self.POSE_X[pose], y0 + rect_y)
            )

    def test_grammar_row_extends_to_seven_cells_ending_in_r0(self) -> None:
        sheet = self.sheet()
        canvas = sheet.build()
        grammar = [
            c for c in sheet.cells
            if c["section"] == "grammar" and c["facing"] == "down"
        ]
        self.assertEqual(
            ["idle", "f1", "a0", "wind", "k0", "lunge", "r0"],
            [c["phase"] for c in grammar],
        )
        self.assertEqual(
            [0, 0, 0, -3, 0, 6, 0], [c["win_px"] for c in grammar]
        )
        r0_cell = grammar[-1]
        x0, y0 = r0_cell["rect"][0], r0_cell["rect"][1]
        # tell_cell: sprite at half (16) + offset (0); r0 rect row 24 -> 40
        self.assertEqual(BODY, canvas.get(x0 + 18, y0 + 16 + 24))

    def test_film_rows_carry_the_eight_column_strip(self) -> None:
        sheet = self.sheet()
        sheet.build()
        for zone in ("zone_1", "zone_2"):
            film = [
                c for c in sheet.cells
                if c["section"] == "film" and c["facing"] == "right"
                and c["zone"] == zone
            ]
            self.assertEqual(list(timeline.STRIP), [c["pose"] for c in film])
            self.assertTrue(all(c["window_tiles"] == 1 for c in film))

    def test_twox_row_shows_timeline_r_boundaries_at_2x(self) -> None:
        sheet = self.sheet()
        sheet.build()
        twox = [c for c in sheet.cells if c["section"] == "twox"
                and c["facing"] == "down"]
        self.assertEqual([23, 24, 31, 32], [c["tick"] for c in twox])
        self.assertEqual(["k0", "r0", "r0", "idle"], [c["pose"] for c in twox])
        self.assertTrue(all(c["scale"] == 2 for c in twox))


class ValidatorTest(SyntheticRecoveryFixture):
    def test_purity_passes_on_a_fresh_build(self) -> None:
        sheet = self.sheet()
        canvas = sheet.build()
        result = recovery_metrics.check_purity(canvas, sheet, self.dirs())
        self.assertEqual([], result["failures"])
        self.assertEqual(len(sheet.cells), result["cells_checked"])

    def test_purity_catches_a_repainted_cell(self) -> None:
        sheet = self.sheet()
        canvas = sheet.build()
        cell = next(
            c for c in sheet.cells
            if c["section"] == "attack" and c["phase"] == "recovery"
        )
        x0, y0 = cell["rect"][0], cell["rect"][1]
        canvas.put(x0 + 5, y0 + 5, (1, 2, 3, 255))
        result = recovery_metrics.check_purity(canvas, sheet, self.dirs())
        self.assertTrue(
            any("region mismatch" in failure for failure in result["failures"])
        )

    def test_purity_catches_export_byte_drift(self) -> None:
        sheet = self.sheet()
        canvas = sheet.build()
        write_rect(
            self.recovery / "player_1_lane_b_attack_down_r0.png", 19, 24, 7, 4
        )
        result = recovery_metrics.check_purity(canvas, sheet, self.dirs())
        self.assertTrue(result["failures"])

    def test_tick_math_passes_and_catches_pin_drift(self) -> None:
        sheet = self.sheet()
        sheet.build()
        self.assertEqual(
            [], recovery_metrics.check_tick_math(sheet, self.reference)
        )
        tampered = json.loads(json.dumps(self.reference))
        tampered["attack_timing"]["values"]["recovery_frames"]["value"] = 9
        failures = recovery_metrics.check_tick_math(sheet, tampered)
        self.assertTrue(any("recovery cells rendered" in f for f in failures))
        tampered = json.loads(json.dumps(self.reference))
        tampered["feedback_states"]["lunge_offset"]["windup_px"] = -4
        failures = recovery_metrics.check_tick_math(sheet, tampered)
        self.assertTrue(any("windup offset" in f for f in failures))

    def test_tick_math_catches_a_wrong_recovery_pose(self) -> None:
        sheet = self.sheet()
        sheet.build()
        for cell in sheet.cells:
            if (cell["section"] == "attack" and cell["timeline"] == "R"
                    and cell["phase"] == "recovery"):
                cell["pose"] = "idle"
        failures = recovery_metrics.check_tick_math(sheet, self.reference)
        self.assertTrue(any("recovery pose" in f for f in failures))

    def test_export_pins_verify_and_fail_on_mismatch(self) -> None:
        import hashlib

        pins = {}
        for pose, (dir_key, name) in recovery_metrics.POSE_FILES.items():
            for facing in timeline.FACINGS:
                file = self.dirs()[dir_key] / name.format(facing=facing)
                pins[file.name] = hashlib.sha256(file.read_bytes()).hexdigest()
        result = recovery_metrics.check_export_pins(self.dirs(), pins)
        self.assertEqual([], result["failures"])
        self.assertEqual(16, result["verified"])
        pins["player_1_lane_b_attack_down_r0.png"] = "0" * 64
        result = recovery_metrics.check_export_pins(self.dirs(), pins)
        self.assertTrue(any("sha256" in f for f in result["failures"]))
        del pins["player_1_lane_b_attack_down_r0.png"]
        result = recovery_metrics.check_export_pins(self.dirs(), pins)
        self.assertTrue(any("no banked release pin" in f for f in result["failures"]))

    def test_report_is_deterministic(self) -> None:
        reports = [
            recovery_metrics.build_report(
                self.recovery, self.anticipation, self.attack, self.walks,
                self.idles, self.reference, verify_pins=False,
            )
            for _ in range(2)
        ]
        self.assertEqual(
            json.dumps(reports[0], sort_keys=True),
            json.dumps(reports[1], sort_keys=True),
        )
        report = reports[0]
        self.assertTrue(report["sheet"]["deterministic"])
        self.assertEqual([], report["purity"]["failures"])
        self.assertEqual([], report["tick_math_failures"])
        self.assertIn(
            "export pins were not verified (skipped)",
            recovery_metrics.check_report(report),
        )

    def test_boundary_profile_characterizes_both_boundaries(self) -> None:
        plan = timeline.build_plan(self.reference)
        boundaries = recovery_metrics.boundary_profile(plan)
        settle = boundaries["release_to_settle"]
        self.assertEqual((23, 24), (settle["from_tick"], settle["to_tick"]))
        self.assertEqual(-6, settle["position_delta_px"])
        self.assertEqual(["k0", "r0"], settle["pose_change"]["R"])
        self.assertEqual(["k0", "idle"], settle["pose_change"]["A"])
        self.assertEqual(["k0", "a0"], settle["pose_change"]["C"])
        ready = boundaries["recovery_end_to_idle"]
        self.assertEqual((31, 32), (ready["from_tick"], ready["to_tick"]))
        self.assertEqual(0, ready["position_delta_px"])
        self.assertEqual(["r0", "idle"], ready["pose_change"]["R"])
        self.assertEqual(["idle", "idle"], ready["pose_change"]["A"])

    def test_durations_report_the_kb_followthrough_band(self) -> None:
        plan = timeline.build_plan(self.reference)
        durations = recovery_metrics.durations_ms(plan["constants"])
        self.assertEqual(133.3, durations["recovery_ms"])
        self.assertEqual(150.0, durations["kb_followthrough_hold_ms"])
        self.assertEqual(-16.7, durations["recovery_vs_kb_band_ms"])

    def test_check_static_flags_each_pre_registered_bar(self) -> None:
        def report_with(**overrides) -> dict:
            metrics = {
                "pose": {"feet_row_delta_vs_idle": 0},
                "walk_confusability_floor_pct": 30.0,
                "coil_distinctness_pct": 30.0,
                "strike_distinctness_pct": 30.0,
                "max_delta_pct": 40.0,
            }
            metrics.update(overrides)
            return {
                "static": {"down": metrics},
                "reference": {"idle_cross_facing": {"popping_pct": 44.44}},
            }

        self.assertEqual([], recovery_metrics.check_static(report_with()))
        self.assertTrue(recovery_metrics.check_static(
            report_with(pose={"feet_row_delta_vs_idle": 2})
        ))
        self.assertTrue(recovery_metrics.check_static(
            report_with(walk_confusability_floor_pct=24.9)
        ))
        failures = recovery_metrics.check_static(
            report_with(coil_distinctness_pct=24.9)
        )
        self.assertTrue(any("grammar inversion" in f for f in failures))
        self.assertTrue(recovery_metrics.check_static(
            report_with(strike_distinctness_pct=24.9)
        ))
        self.assertTrue(recovery_metrics.check_static(
            report_with(max_delta_pct=44.44)
        ))


class SpecContractTest(unittest.TestCase):
    """The r0 specs carry the declared byte-exact head machinery."""

    HEAD = {
        "down": ((4, 14), (2, 3)),
        "right": ((4, 9), (1, 4)),
    }

    def spec_pair(self, facing: str):
        idle = load_spec(
            REPO / "sources" / "calibration-v0" / "specs"
            / f"player_1_lane_b_idle_{facing}.json"
        )
        r0 = load_spec(
            REPO / "sources" / "calibration-v5" / "specs"
            / f"player_1_lane_b_attack_{facing}_r0.json"
        )
        return idle, r0

    def test_head_block_is_byte_exact_at_the_declared_translation(self) -> None:
        for facing, ((row0, row1), (dx, dy)) in self.HEAD.items():
            idle, r0 = self.spec_pair(facing)
            for y in range(row0, row1 + 1):
                for x, char in enumerate(idle.grid[y]):
                    if char == ".":
                        continue
                    self.assertEqual(
                        char, r0.grid[y + dy][x + dx],
                        f"{facing}: head pixel ({x},{y}) not exact at ({dx},{dy})",
                    )

    def test_ramp_is_the_frozen_five_color_palette(self) -> None:
        for facing in ("down", "right"):
            idle, r0 = self.spec_pair(facing)
            self.assertEqual(idle.palette, r0.palette)
            self.assertEqual(idle.used_colors, r0.used_colors)

    def test_no_jaw_gape_authored(self) -> None:
        # The gape is k0's exclusive marker: a horizontal accent run wider
        # than the feet caps (3px). r0 must never author one.
        for facing in ("down", "right"):
            _, r0 = self.spec_pair(facing)
            for y, row in enumerate(r0.grid):
                longest = max(
                    (len(run) for run in row.split(".") for run in run.split("o")
                     if set(run) == {"k"}),
                    default=0,
                )
                self.assertLessEqual(
                    longest, 3, f"{facing}: accent run {longest} at row {y}"
                )

    def test_feet_rest_on_the_idle_contact_row(self) -> None:
        for facing in ("down", "right"):
            idle, r0 = self.spec_pair(facing)
            self.assertEqual(idle.bbox[3], r0.bbox[3])


class RealArtifactsTest(unittest.TestCase):
    """The real banked exports drive the sheet; committed artifacts stay stable."""

    def setUp(self) -> None:
        if not (REPO / "exports" / "calibration-v5"
                / "player_1_lane_b_attack_down_r0.png").is_file():
            self.skipTest("calibration-v5 exports not present")
        self.reference = load_reference()

    def build_sheet(self) -> timeline.RecoveryTimelineSheet:
        return timeline.RecoveryTimelineSheet(
            REPO / "exports" / "calibration-v5", REPO / "exports" / "calibration-v3",
            REPO / "exports" / "calibration-v2", REPO / "exports" / "calibration-v1",
            REPO / "exports" / "calibration-v0", self.reference,
        )

    def test_real_build_passes_the_full_check(self) -> None:
        if not (REPO / "exports" / "calibration-v5" / "release.json").is_file():
            self.skipTest("calibration-v5 release not banked yet")
        report = recovery_metrics.build_report(
            REPO / "exports" / "calibration-v5", REPO / "exports" / "calibration-v3",
            REPO / "exports" / "calibration-v2", REPO / "exports" / "calibration-v1",
            REPO / "exports" / "calibration-v0", self.reference,
            sheet_path=REPO / "reviews" / "calibration-v5" / "timeline-sheet.png",
        )
        failures = recovery_metrics.check_report(report)
        self.assertEqual([], failures)
        self.assertEqual(16, report["export_pins"]["verified"])

    def test_banked_recovery_sheet_regenerates_byte_identical(self) -> None:
        sheet_path = REPO / "reviews" / "calibration-v5" / "timeline-sheet.png"
        if not sheet_path.is_file():
            self.skipTest("calibration-v5 sheet not banked yet")
        self.assertEqual(sheet_path.read_bytes(), self.build_sheet().build().encode())

    def test_banked_apng_aids_regenerate_byte_identical(self) -> None:
        apng_dir = REPO / "reviews" / "calibration-v5"
        if not (apng_dir / "timeline-arc-down.apng").is_file():
            self.skipTest("calibration-v5 apng aids not banked yet")
        sheet = self.build_sheet()
        for facing in timeline.FACINGS:
            frames = timeline.build_apng_frames(sheet, facing)
            payload = timeline.encode_apng(frames, timeline.apng_delays(len(frames)))
            self.assertEqual(
                (apng_dir / f"timeline-arc-{facing}.apng").read_bytes(), payload,
                f"apng {facing} drifted",
            )

    def test_sheet_main_writes_sheet_and_apng_aids(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            out = Path(temp) / "sheet.png"
            status = timeline.main(["--out", str(out), "--apng-dir", temp])
            self.assertEqual(0, status)
            self.assertTrue(out.is_file())
            for facing in timeline.FACINGS:
                self.assertTrue(
                    (Path(temp) / f"timeline-arc-{facing}.apng").is_file()
                )

    def test_metrics_main_checks_the_real_chain_end_to_end(self) -> None:
        if not (REPO / "exports" / "calibration-v5" / "release.json").is_file():
            self.skipTest("calibration-v5 release not banked yet")
        with tempfile.TemporaryDirectory() as temp:
            sheet_path = Path(temp) / "sheet.png"
            status = timeline.main(["--out", str(sheet_path)])
            self.assertEqual(0, status)
            out = Path(temp) / "metrics.json"
            status = recovery_metrics.main(
                ["--sheet", str(sheet_path), "--out", str(out), "--check"]
            )
            self.assertEqual(0, status)
            report = json.loads(out.read_text(encoding="utf-8"))
            self.assertTrue(report["sheet"]["committed_matches"])
            # corrupting the committed sheet must fail the determinism bar
            payload = bytearray(sheet_path.read_bytes())
            payload[-20] ^= 0xFF
            sheet_path.write_bytes(bytes(payload))
            status = recovery_metrics.main(
                ["--sheet", str(sheet_path), "--out", str(out), "--check"]
            )
            self.assertEqual(1, status)

    def test_metrics_main_reports_missing_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            status = recovery_metrics.main(
                ["--idle-exports", str(Path(temp) / "missing"),
                 "--out", str(Path(temp) / "r.json")]
            )
            self.assertEqual(1, status)


if __name__ == "__main__":
    unittest.main()
